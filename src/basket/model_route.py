...
def transaction_order(db_config: dict, sql_provider, basket: dict, user_id: int):
    ...
    status, res_dict = select_line(db_config, _sql)
    if not status:
    ...
    for key, value in basket.items():
        ...
        if not result[0]:
            ...
            delete(db_config, _sql)
            return ProductInfoRespronse(value, error_message="Не удалось вставить order_line", status=False)

    _sql = sql_provider.get('update_order.sql', total_cost=total_cost, e_order_id=order_id)
    status, msg = update(db_config, _sql)
    if not status:
